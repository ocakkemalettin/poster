...
#[tauri::command]
pub fn build_report_csv(report: &crate::load_test::LoadTestReport) -> String {
    let mut out = String::from("seq,status,latency_ms,success\n");
...
#[tauri::command]
pub fn build_summary_csv(report: &crate::load_test::LoadTestReport) -> String {
    // 1-second time-slice summary
    let mut buckets: HashMap<u64, (u64, u64, u64)> = HashMap::new();
    for r in &report.per_request {
        let _ = r;
    }
    // per-request has no timestamp; approximate by seq order over duration
    let n = report.per_request.len() as f64;
    let dur = report.duration_ms.max(1) as f64;
    for (i, r) in report.per_request.iter().enumerate() {
        let t = if n > 0.0 {
            ((i as f64 + 0.5) / n * dur) as u64 / 1000
        } else {
            0
        };
        let b = buckets.entry(t).or_insert((0, 0, 0));
        b.0 += 1;
        if r.success {
            b.1 += 1;
        } else {
    ...
    let mut out = String::from("second,total,successes,failures\n");
